# Lessons — the narratives behind the rules in CLAUDE.md

`CLAUDE.md` is a reference: it describes the system as it is now and states the rules that
govern changes to it. This file is where those rules came from — the runs that produced them,
what was measured, what was tried and rejected, and the dates. Nothing here is required
reading before making a change; it is what you consult when a rule looks arbitrary, when you
are about to argue with one, or when you hit a failure that smells familiar.

Two sections, lifted verbatim from `CLAUDE.md` on 2026-09-02 when it was compressed from 945
lines to fit the maintainer's ~500-line working limit. The rules they produced all still live
in `CLAUDE.md`; only the stories moved.

- [Project state and the backfill track](#project-state-and-the-backfill-track) — the daily
  cron pause, the per-candidate backfill track (#50), and the full "what #X settled" write-up
  for every candidate run so far.
- [Non-obvious lessons](#non-obvious-lessons-paid-for-in-real-runs) — bugs paid for in real
  runs, each with what was measured and why the obvious fix was wrong.

---

## Project state and the backfill track

_Verbatim from `CLAUDE.md`'s "Known gaps / planned work" section as of 2026-09-02. The
present-tense version of the state described here is in `CLAUDE.md`; treat this copy as the
historical record, not as current status._
